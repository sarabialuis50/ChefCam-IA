...
const ScannerView: React.FC<ScannerViewProps> = ({ onCancel, onComplete, onReadyToGenerate }) => {
  ...
  const handleScan = async () => {
    ...
    if (context) {
      ...
      try {
        const ingredients = await analyzeIngredientImage(base64);
        clearInterval(interval);
        setProgress(100);
        setStatus("Identificación completa");
        onComplete(ingredients.map(i => i.name));
  ...
  return (
    <div className="h-screen w-full bg-pure-black relative flex flex-col items-center justify-between p-6 overflow-hidden">
      ...
      <header className="relative z-10 w-full flex justify-start pt-6">
        <button onClick={onCancel} className="p-3 rounded-full bg-black/40 border border-white/10">
      ...
      <div className="relative z-10 flex-1 w-full flex flex-col items-center justify-center -mt-16">
        <div className="relative w-72 h-72 flex items-center justify-center">
          ...
          <div className="relative flex flex-col items-center gap-4">
            <div className={`w-20 h-20 rounded-full border border-primary/20 ${!cameraError && !scanning ? 'animate-pulse-slow' : ''} flex items-center justify-center`}>
              <span className="material-symbols-outlined text-primary text-5xl neon-text-glow">smart_toy</span>
            </div>
            ...
            <div className="bg-surface-dark border border-primary/30 px-4 py-1.5 rounded-full flex items-center gap-2 shadow-xl">
              ...
              <span className="text-[10px] font-bold tracking-widest text-primary uppercase font-tech">ChefCam Engine v2.5</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 text-center space-y-2 px-4">
          <h2 className="text-2xl font-tech font-bold text-white tracking-tight">
            {scanning ? "Codificando datos..." : cameraError ? "Error de Sistema" : "Escaneo Biométrico"}
          </h2>
          <p className="text-sm text-gray-400 max-w-xs">{status}</p>
        </div>
      </div>

      <div className="relative z-10 w-full pb-12 space-y-6">
        {scanning ? (
          ...
          <div className="flex flex-col gap-4">
            <button 
              ...
              className={`w-full h-16 rounded-[2rem] font-black text-lg flex items-center justify-center gap-3 transition-all ${
                cameraError ? 'bg-zinc-800 text-zinc-600 cursor-not-allowed' : 'bg-primary text-black shadow-strong active:scale-95'
              }`}
            >
